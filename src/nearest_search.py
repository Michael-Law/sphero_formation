...
class NearestSearch(object):
    ...
    to individual agents alongside the list of obstacles within range.
    """
	...
	#self.map_depth = data.info.depth
	...
	#depth = (self.map_origin.z + self.map_height *
               #self.map_resolution - z_real) / self.map_resolution
        return int(col), int(row)#, int(depth)
	...
	#z_real = self.map_origin.z + (self.map_height - row) * self.map_resolution
    ...
    def param_callback(self, data):
        ...
        # TODO: Comment
        # I removed the avoid radius here because it makes more sense to use for
        # minimum distance from the obstacle, and all obstacles are sought in sight
        # agent field defined with search radius
    ...
    def robot_callback(self, *data):
        ...
        for agent in data:
            ...
            for agent2 in data:
                ...
                if distance > 0 and distance <= self.search_radius:
		            ...
		            # rel_pos.pose.pose.position.z = friend_position.z - agent_position.z
	    ...
	    #depth_range = range(max(0, or_depth - self.r), min(self.map_depth, or_depth + self.r + 1))
            for row in row_range:
			    ...
			    #obst.position.z = z - agent_position.z
...
if __name__ == '__main__':
    rospy.init_node('NearestSearch')   #Initiate a node named'NearestSearch'.
